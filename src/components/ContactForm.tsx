...
import { CONTACT_LIMITS, validateContact, type ContactErrors, type ContactField } from '@/lib/contact';
...
export function ContactForm() {
  ...
  return (
    <form
      ...
      noValidate
      onSubmit={onSubmit}
      onFocus={() => void turnstile()?.prime().catch(() => undefined)}
    ...
    >
      ...
      {status === 'failed' ? (
        <p role="alert" className="text-sm text-ink-strong">
          ...
          <a className="font-medium underline underline-offset-4 hover:text-accent" href={`mailto:${site.email}`}>
            {site.email}
          </a>{' '}
          instead.
        </p>
